...
static const struct  wl2866d_map  wl2866d_on_config[] = {
	...
	{0x04, 0x54},//OUT_DVDD2 1.1V -> Wide
	{0x05, 0x81},//OUT_AVDD1 2.8V -> Front/UW/depth/macro
	{0x06, 0x88},//OUT_AVDD2 0x80->2.8V/0x88->2.9V ->Wide TODO:use 0x84/2.85 for debug //0x88
	{0x0E, 0x0F},//VOL_ENABLE
	{0x0E, 0x00},//VOL_DISABLE
};
//For 64M
static const struct  wl2866d_map  wl2866d_on_config_64M[] = {
	{0x03, 0x64},//OUT_DVDD1 1.2V -> Front/UW
	{0x04, 0x54},//OUT_DVDD2 1.1V -> Wide
	{0x05, 0x81},//OUT_AVDD1 2.8V -> Front/UW/depth/macro
	{0x06, 0x80},//OUT_AVDD2 0x80->2.8V/0x88->2.9V ->Wide
	{0x0E, 0x0F},//VOL_ENABLE
	{0x0E, 0x00},//VOL_DISABLE
};
...
{
    int ret = -1;
	ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[VOL_ENABLE].reg, 0);
	if (ret < 0) {
				 ...
				 const struct i2c_device_id *id)
{
    ...
    ret = wl2866d_i2c_write(chip, 0x10, 1);
    if (ret < 0) {
        pr_err("ET5904/wl2866d init voltage failed %d\n", __LINE__);
        return ret;
    }
    wl2866d_i2c_read(chip, 0x10, &reg_val);
    pr_err("%s:ET5904/wl2866d ldo module info after is %d\n", __func__, reg_val);
    if (1 == reg_val) {
        pr_err("%s: ET5904/wl2866d ldo module info is ET5904\n", __func__);
    } else {
        pr_err("%s: ET5904/wl2866d ldo module info is wl2866d\n", __func__);
    }
    ret = wl2866d_i2c_write(chip, 0x10, 0);
    if (ret < 0) {
        pr_err("ET5904/wl2866d init voltage failed %d\n", __LINE__);
        return ret;
    }
    wl2866d_i2c_read(chip, 0x10, &reg_val);
    pr_err("%s:ET5904/wl2866d ldo module info last is %d\n", __func__, reg_val);
